...
for (col in colnames(categorical_data)) {
  ...
  # Pie chart
  pie_plot <- ggplot(plot_df, aes(x = "", y, fill = x)) +
    geom_bar(stat = "identity", width = 1) +
    coord_polar("y", start = 0) +
    labs(x = NULL, y = NULL, fill = col) +
    theme_void() +
    ggtitle(paste("Pie Chart of", col))
  
  # Bar chart
  bar_plot <- ggplot(plot_df, aes(x, y)) +
    geom_bar(stat = "identity", fill = "steelblue", width = 0.5) +
    labs(x = col, y = "Count") +
    ggtitle(paste("Bar Plot of", col))
  
  # Combine pie chart and bar chart using patchwork
  combined_plot <- pie_plot + bar_plot + plot_layout(guides = "collect")
  
  combined_plots[[col]] <- combined_plot
}

# Combine all plots in one output
final_output <- wrap_plots(combined_plots, ncol = 2)
...
```{r}
T1_data$productLetterGrade_encoded <- ifelse(T1_data$productLetterGrade == "F", 0, 1)
...
# Add the encoded column to numeric_data_no_outliers
numeric_data_no_outliers$teamLeadGender <- T1_data_subset$teamLeadGender_encoded
...
```{r}
selected_columns <- c("femaleTeamMembersPercent", "inPersonMeetingHoursTotal", "LD1", "commitMessageLengthTotal", "teamLeadGender","teamDistribution_encoded", "productLetterGrade_encoded")
