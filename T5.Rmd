...
T5_data <- read_excel(csv_file, sheet = 'setapProductT5')
...
T5_data <- T5_data[, columns_to_keep]
T5_data$globalLeadAdminHoursTotal[is.na(T5_data$globalLeadAdminHoursTotal)] <- 0


#Add column of encoded team dist
T5_data$teamDistribution_encoded <- ifelse(T5_data$teamDistribution == "Local", 0, 1)
print(T5_data)
...
```{r}
gender_counts <- table(T5_data$teamLeadGender)
gender_counts
```
```{r}
...
numeric_data <- T5_data[ , numeric_columns]
```
...
for (col in colnames(numeric_data)) {
  ...
  filename <- paste0("boxplot_plotsT5/", col, "_boxplotT5.png")
...
dir.create("density_plots_distT5")
...
for (col in colnames(numeric_data)) {
  ...
  filename <- paste0("density_plots_distT5/", col, "_density_plot_T5_", unique(numeric_data$teamDistribution_encoded), ".png")
  ggsave(filename, plot, width = 6, height = 4, dpi = 300)
