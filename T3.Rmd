...
T1_data <- read_excel(csv_file, sheet = 'setapProductT3')
...
T3_data <- T3_data[, columns_to_keep]
T3_data$globalLeadAdminHoursTotal[is.na(T3_data$globalLeadAdminHoursTotal)] <- 0


#Add column of encoded team dist
T3_data$teamDistribution_encoded <- ifelse(T3_data$teamDistribution == "Local", 0, 1)
print(T3_data)
...
```{r}
gender_counts <- table(T3_data$teamLeadGender)
gender_counts
```
```{r}
...
numeric_data <- T3_data[ , numeric_columns]
```
...
for (col in colnames(numeric_data)) {
  ...
  filename <- paste0("boxplot_plotsT3/", col, "_boxplotT3.png")
...
dir.create("density_plots_distT3")
...
for (col in colnames(numeric_data)) {
  ...
  filename <- paste0("density_plots_distT3/", col, "_density_plot_T3_", unique(numeric_data$teamDistribution_encoded), ".png")
  ggsave(filename, plot, width = 6, height = 4, dpi = 300)
